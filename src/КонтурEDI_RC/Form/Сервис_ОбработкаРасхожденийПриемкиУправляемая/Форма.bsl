...
Функция ПеренестиВДокументРеализацияСервер()
	...
	МодульОбъекта().ОбработкаСобытияПодключаемогоМодуля("ОбработатьРасхожденияРезультатовПриемки",СтандартнаяОбработкаEDI,Новый Структура("Накладная,ТаблицаРасхождений",Документ,ТабСоответствий));
	...
	Если СтандартнаяОбработкаEDI = Истина Тогда
		...
		//рефакторинг оживит работу с компонентой диадок - все делать на клиенте
		...
		//		Если ЗначениеЗаполнено(boxId) И ЗначениеЗаполнено(messageId) И ЗначениеЗаполнено(invoiceId) и ЗначениеЗаполнено(torg12Id) Тогда
		//			Попытка
		...
		//			Исключение
		//			КонецПопытки;
		//			Если ДокументДД_ТОРГ12.Status 		  = "OutboundRecipientSignatureRequestRejected" ИЛИ // отказано в подписании накладной
		//				 ДокументДД_СФ.AmendmentRequested = Истина Тогда 									// если запрошено уточнение
		...
		МодульОбъекта().ОбработкаСобытияПодключаемогоМодуля("ПослеОбработкиРасхожденияРезультатовПриемки",СтандартнаяОбработкаEDI,Новый Структура("Накладная,ВыбранноеДействие,ТаблицаРасхождений",Документ,"Реализация",ТабСоответствий));
...
Функция СоздатьВозвратНаСервере()
	...
	Если СписокОшибок="" Тогда
		...
		СтандартнаяОбработкаEDI=Истина;
		МодульОбъекта().ОбработкаСобытияПодключаемогоМодуля("ПослеОбработкиРасхожденияРезультатовПриемки",СтандартнаяОбработкаEDI,Новый Структура("Накладная,ВыбранноеДействие,ТаблицаРасхождений",ДокументВозврата.Ссылка,"Возврат",ТабСоответствий));
...
Процедура ИгнорироватьРасхожденияСервер()
	...
	МодульОбъекта().УстановитьСтатусДокумента(Документ,"Принят частично (обработан)","Приемка",Параметры.СообщениеСсылка);
	МодульОбъекта().ОбработкаСобытияПодключаемогоМодуля("ПослеОбработкиРасхожденияРезультатовПриемки",,Новый Структура("Накладная,ВыбранноеДействие,ТаблицаРасхождений,СообщениеСсылка",Документ,"Игнорировать",Неопределено,Параметры.СообщениеСсылка));
