...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Лев(НомерРелизаПоследнихОбновлений,8)<>Лев(НомерРелизаМодуля,8) Тогда
...
Процедура ОбработчикРешенияПользователяОПродолженииПереходаНаНовуюРедакцию(РезультатВопросаОПродолжении,ДопПараметры=неопределено) Экспорт
	...
	Иначе
		 ...
		 Если Параметры.МодальностьЗапрещена Тогда 
		 	Выполнить("ПоказатьПредупреждение(Новый ОписаниеОповещения(""ОбработчикОзнакомленияСТребованиемСоздатьРезервнуюКопию"", ЭтаФорма, ДопПараметрДляПередачиВОбработчик),ТекстПредупреждения,,""EDI.Контур"")");
		 Иначе
...
Процедура ВывестиОшибкиМетаданныхEDI(Отказ,ДопПараметры=неопределено) Экспорт
	...
	//Если истина Тогда 
		...
		Для Каждого Ошибка из СписокОшибок ЦИкл
			...
			Если Найти(Ошибка.Значение,"В конфигурации отсутствует")<>0 Тогда 
		...
		//Если истина Тогда 
			ТекстПредупреждения="Для работы с модулем Контур.EDI необходимо обновить конфигурацию!";	
		...
		Если Параметры.МодальностьЗапрещена Тогда 
			Выполнить("ПоказатьПредупреждение(Новый ОписаниеОповещения(""ОбработчикОтказаОткрытияЭтойФормы"", ЭтаФорма,),ТекстПредупреждения,,""EDI.Контур"")");
		Иначе
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ РезультатПроверкиЗапускНовойВерсииМодуля.ПродолжитьЗапуск Тогда
		Если Параметры.МодальностьЗапрещена Тогда
			Выполнить("ПоказатьПредупреждение(Новый ОписаниеОповещения(""ОбработчикОтказаОткрытияЭтойФормы"", ЭтаФорма,),РезультатПроверкиЗапускНовойВерсииМодуля.ТекстОшибки,,""EDI.Контур"")");
		Иначе
...
Процедура ОбработчикПослеРаботыСтартовогоПомощника(РезультатРаботыСтартовогоПомощника, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		...
		Если ЭтоПервыйЗапуск = истина Тогда 
			...
			Если Параметры.МодальностьЗапрещена Тогда
				Выполнить("ПоказатьВопрос(Новый ОписаниеОповещения(""ОбработчикСогласияНаПервыйОбмен"", ЭтаФорма, ДопПараметрДляПередачиВОбработчик), ТекстВопроса, КнопкиВопроса,,,""EDI.Контур"")");
			Иначе
...
Процедура УстановитьЗаголовокФормы()
	...
	ТекстЗаголовкаФормы = "EDI.Контур "+НомерРелизаМодуля+" / ";
...
Процедура ВыполнитьЗадачу(ВыбраннаяСтрока,ПакетнаяОбработка=Ложь)
	...
	Иначе	
		...
		ИначеЕсли КодДействия = "РаботаЗавершена"
			Или КодДействия = "ДождатьсяДоставки"	Тогда
...
Функция _ПереотправитьСообщение(ВыбраннаяСтрока,НеОткрыватьФормы = Ложь,КодДействия="")
	...
	Если НеОткрыватьФормы Тогда
		...
		//обработаем и вычеркнем если все ок
		Результат = ОтправитьИсходящееСообщениеБезОткрытияФормыСервер(ВыбраннаяСтрока.ТипСообщения, ВыбраннаяСтрока.Документ);
		Возврат Результат;
...
&НаКлиенте
Процедура мОткрытьКарточкуСообщения(СтрокаСообщения,КодДействия="",ПроизвольныеПараметры=Неопределено)  //!!! В управляемых формах несколько иной механизм сбора исходящих сообщения.
	...
	Если СтрокаСообщения.ТребуемоеДействие = "Переотправить сообщение"
		или (СтрокаСообщения.ТребуемоеДействие = "Работа завершена" и Лев(КодДействия,10)="Отправить_")  Тогда
	...
	Если КодДействия="Отправить_ORDRSP" и ТипЗНЧ(ПроизвольныеПараметры)=Тип("Структура") и ПроизвольныеПараметры.Свойство("Статус") и ПроизвольныеПараметры.Статус = "Отклонить" Тогда 
		ПараметрыФормы.ТипСообщения	= "ORDRSP";  //костылик для отправки полной отмены в ordrsp из списка
	КонецЕсли;
	...
	Если СтрокаСообщения.ТипСообщения = "PRICELIST" Тогда
		ИмяКарточкиСообщения = "ОтправкаPriceListУправляемая";
	КонецЕсли;
...
Процедура КнопкаЕщеСвязанныеСообщения(Команда)
	...
	Если ТекСтрока<>Неопределено
		...
		ПараметрыФормы.Вставить("СообщениеСсылка",Неопределено);//вроде не имеет смысла
...
Процедура КнопкаЕщеОтклонитьЗаказПолностью(Команда)
	...
	ПроизвольныеПараметры.Вставить("ВходящийЗаказ", ТекСтрока.СообщениеСсылка);   //теперь так можно
	
	МожноОтклонитьЗаказ = МожноОтправитьORDRSP(ТекСтрока.Партнер);
	...
	Иначе
		...
		Если Параметры.МодальностьЗапрещена Тогда
			Выполнить("ПоказатьВопрос(Новый ОписаниеОповещения(""ОбработчикСогласияНагрупповыеORDRSP"", ЭтаФорма, ДопПараметрДляПередачиВОбработчик), ТекстВопроса, КнопкиВопроса,,,""EDI.Контур"")");
		Иначе
			РезультатВопроса = Вопрос(ТекстВопроса, КнопкиВопроса,,,"EDI.Контур");
...
&НаСервере
Функция МожноОтправитьORDRSP(Партнер)
//Функция МожноОтправитьORDRSP(СообщениеСсылка)
	//заново переопределим Партнера т.к. на момент принятия сообщения партнер мог быть иным (или не определенным)
	//СообщениеЗаказ = МодульОбъекта().ПрочитатьСообщение(СообщениеСсылка,,"ORDERS","Входящее");
	//ТекПартнер = МодульОбъекта().ОпределитьПартнера(Новый Структура("ТипЗначения,Сообщение","Сообщение",СообщениеЗаказ));
	Возврат (МодульОбъекта().ПолучитьЗначениеСвойстваОбъектаEDI(Партнер,"ORDRSP")=Истина);
	
КонецФункции // МожноОтправитьORDRSP()
...
Процедура ВыполнитьВыбранныеЗадачи(Команда)
	...
	_РежимГрупповойОбработки = (Элементы.ТабСообщения.ВыделенныеСтроки.Количество()>1);
	//смотрим, сколько строк выбрал пользователь. Если несколько, то не будем обновлять список автоматически.
	
	//ОтборПоТребуемымДействиям = ПолучитьТребуемыеДействияОтбора(Ложь);
	//Если ОтборПоТребуемымДействиям.Количество()>1 Тогда
	//	_РежимГрупповойОбработки = Истина;
	//	ВывестиПредупреждение_КонтурEDI("Для выполнения групповых действий установите отбор по одному требуемому действию слева");
	//	Возврат;
	//Иначе
	//	_РежимГрупповойОбработки = Ложь;
	//КонецЕсли;	
	...
	Иначе
		...
		Если Параметры.МодальностьЗапрещена Тогда
			Выполнить("ПоказатьВопрос(Новый ОписаниеОповещения(""ОбработчикСогласияНагрупповыеДействия"", ЭтаФорма, ДопПараметрДляПередачиВОбработчик), ТекстВопроса, КнопкиВопроса,,,""EDI.Контур"")");
		Иначе
			РезультатВопроса = Вопрос(ТекстВопроса, КнопкиВопроса,,,"EDI.Контур");
...
Процедура ОбработчикСогласияНаГрупповыеДействия(РезультатВопроса,ДопПараметрПереданныйВОбработчик=Неопределено) Экспорт
	...
	//Если ОтборПоТребуемымДействиям[0] = "Выполнить обмен" Тогда
	//	КоманднаяПанельСообщенияВыполнитьОбмен("");
	//	Возврат;
	//КонецЕсли;
	
	//СтрокаОформленияТребуемогоДействия = НастройкаОформленияТребуемогоДействия.Найти(ОтборПоТребуемымДействиям[0],"Значение");
	//Если СтрокаОформленияТребуемогоДействия <> Неопределено Тогда
	//	Если Не СтрокаОформленияТребуемогоДействия.Гиперссылка Тогда
	//		Возврат; //тут нечего делать. Это "Обрабатывается сервером", "Ожидайте ответа от торговой сети" или "Работа завершена".
	//	КонецЕсли;
	//КонецЕсли;	
	
	//ОбновлятьВесьСписокПриРаботеСЗаказами				= ПолучитьКонстантуEDI("ОбновлятьВесьСписокПриРаботеСЗаказами");
	//Если ОбновлятьВесьСписокПриРаботеСЗаказами = Истина 
	//	И Не _РежимГрупповойОбработки Тогда
	//	ОбновитьСписокЗадач();
	//	//обновим список. Иначе - зачеркнем текущую строку. При групповой обработке оставляем результат выполнения на экране до ручного обновления
	//КонецЕсли;	
	  //пока решили обновлять в любом случае
	//ЗаполнитьСписокЗадачКлиент();
КонецПроцедуры
...
Процедура ЗапроситьСертификатИПодписатьСФ()
	...
	Если ДоступныеСертификаты.Количество()=0 Тогда
		ТекстПредупреждения="Не обнаружены активные сертификаты для подписания";
		Если Параметры.МодальностьЗапрещена Тогда 
			Выполнить("ПоказатьПредупреждение(,ТекстПредупреждения,,""EDI.Контур"")");
		Иначе
			Предупреждение(ТекстПредупреждения,,"EDI.Контур");
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Режим",		"ВыборИзСписка");
	ПараметрыФормы.Вставить("Заголовок",	"Выберите сертификат для подписания");
	ПараметрыФормы.Вставить("СписокВыбора",	ДоступныеСертификаты);
	
	Если Параметры.МодальностьЗапрещена Тогда 
		Выполнить("ОткрытьФормуОбъектаМодально(ПутьКФормам + ""УниверсальныйДиалогУправляемая"", ПараметрыФормы,""ПослеВыбораСертификатаПользователем"")");//,ДополнительныеПараметры);
	Иначе
		ВыбранноеЗначение = ПолучитьФормуОбработки("УниверсальныйДиалогУправляемая",ПараметрыФормы).ОткрытьМодально();
...
&НаКлиенте
Процедура ПослеВыбораСертификатаПользователем(ВыбранныйЭлемент,Парам2=Неопределено) Экспорт
...
&НаКлиенте
Процедура ИнициализироватьРаботуСДиадоком()
...
&НаКлиенте
Процедура ЗапуститьАвтотесты(ПараметрыЗакрытияИнтерактивнойФормы=Неопределено, ДополнительныеПараметры=Неопределено) Экспорт
...
Процедура ДеревоФильтровВыбор(Элемент, ЗНАЧ ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НЕ ВыбраннаяСтрока = Неопределено Тогда
		...
		ИначеЕсли ГруппаНастроек = "ТипыСообщений" и ВыбраннаяСтрока<>Неопределено и ВыбраннаяСтрока.Значение = "PRICELIST" и Элемент.ТекущийЭлемент.Имя = "ДеревоФильтровИзменить" Тогда 
			//Нажатие на добавление нового прайс листа
            ОткрытьФормуОтправкиPricelist();
		Иначе
...
&НаКлиенте
Процедура ОткрытьФормуОтправкиPricelist()
	
	ПараметрыФормы=	Новый Структура;
	Если Параметры.МодальностьЗапрещена Тогда 
		Выполнить("ОткрытьФормуОбъектаМодально(ПутьКФормам + ""ОтправкаPriceListУправляемая"", ПараметрыФормы,""ОбработчикПослеОтправкиPricelist"")");//,ДополнительныеПараметры);
	Иначе
		ПолучитьФормуОбработки("ОтправкаPriceListУправляемая",ПараметрыФормы).ОткрытьМодально();
		ОбработчикПослеОтправкиPricelist();
	КонецЕсли;
	
КонецПроцедуры 

&НаКлиенте
Процедура ОбработчикПослеОтправкиPricelist(Параметр1=неопределено,ДопПараметр=неопределено) Экспорт
...
Процедура ПредложитьОбновитьОбъектыМетаданныхКонтурEDI(ИмяМакетаОбновления = Неопределено,ТекстВопроса = Неопределено) Экспорт
	Если ЗначениеЗаполнено(ТекстВопроса) Тогда
		...
		Если Параметры.МодальностьЗапрещена Тогда
			Выполнить("ПоказатьВопрос(Новый ОписаниеОповещения(""ОбработчикСогласияСохраненияФайла"", ЭтаФорма, ДопПараметрДляПередачиВОбработчик), ТекстВопроса, КнопкиВопроса,,,""EDI.Контур"")");
		Иначе
			РезультатВопроса = Вопрос(ТекстВопроса, КнопкиВопроса,,,"EDI.Контур");
...
&НаСервере
Процедура ЗаполнитьИдентификаторыДД_Сервер(Документ,boxId,messageId,invoiceId,torg12Id)

	boxId = СокрЛП(МодульОбъекта().ПолучитьЗначениеСвойстваОбъектаEDI(Документ, "boxId"));
	messageId = СокрЛП(МодульОбъекта().ПолучитьЗначениеСвойстваОбъектаEDI(Документ, "messageId"));
	invoiceId = СокрЛП(МодульОбъекта().ПолучитьЗначениеСвойстваОбъектаEDI(Документ, "invoiceId"));
	torg12Id = СокрЛП(МодульОбъекта().ПолучитьЗначениеСвойстваОбъектаEDI(Документ, "torg12Id"));
...
Процедура КнопкаЕщеОткрытьЭлДокументыВДиадоке(Команда)
	...
	ЗаполнитьИдентификаторыДД_Сервер(ТекСтрока.Документ,boxId,messageId,invoiceId,torg12Id);
...
Процедура КнопкаЕщеПроверитьНаличиеОтказовЗапросовНаУточнение(Команда)
	...
	Если МассивДляУточнения.Количество()>0 Тогда 
		Если УстановитьСоединениеСДиадокомПоУмолчанию() Тогда
			ДокументыСУточнениямиОтказами=ЗаполнитьИдентификаторыЧерезКомпонентуДД(МассивДляУточнения); //кл
			
			Если ДокументыСУточнениямиОтказами.Количество()=0 Тогда
				ТекстПредупреждения="По текущему списку счетов-фактур нет новых запросов на уточнение.";
				Если Параметры.МодальностьЗапрещена Тогда 
					Выполнить("ПоказатьПредупреждение(,ТекстПредупреждения,,""EDI.Контур"")");
				Иначе
					Предупреждение(ТекстПредупреждения,,"EDI.Контур");
			...
			Иначе
				ПараметрыФормы=	Новый Структура;
				ПараметрыФормы.Вставить("МассивДокументовСУточнениями",ДокументыСУточнениямиОтказами);
				ПараметрыФормы.Вставить("МодальностьЗапрещена",Параметры.МодальностьЗапрещена);
				
				Если Параметры.МодальностьЗапрещена Тогда 
					Выполнить("ОткрытьФормуОбъектаМодально(ПутьКФормам + ""Сервис_Диадок_ПроверкаУточненийСФУправляемая"", ПараметрыФормы,""ОбновитьСписок"")");//,ДополнительныеПараметры);
				Иначе
					ПолучитьФормуОбработки("Сервис_Диадок_ПроверкаУточненийСФУправляемая",ПараметрыФормы).ОткрытьМодально();
					ОбновитьСписок();
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Функция ПолучитьДокументыСУточнениями()
	...
	Для Каждого Стр ИЗ Объект.ТабСообщения Цикл
		Если СокрЛП(Стр.ТребуемоеДействие) = "Работа завершена" 
			...
			ЗаполнитьИдентификаторыДД_Сервер(Стр.Документ,boxId,messageId,invoiceId,torg12Id);
...
&НаКлиенте
Функция ЗаполнитьИдентификаторыЧерезКомпонентуДД(МассивСтруктурИдентификаторов) Экспорт
	...
	Для Каждого СтруктураИдентификаторов ИЗ МассивСтруктурИдентификаторов ЦИкл
		...
		Если ЗначениеЗаполнено(СтруктураИдентификаторов.boxId) И ЗначениеЗаполнено(СтруктураИдентификаторов.messageId) И (ЗначениеЗаполнено(СтруктураИдентификаторов.invoiceId) или ЗначениеЗаполнено(СтруктураИдентификаторов.torg12Id)) Тогда
...
Процедура КнопкаЕщеПереотправитьПоРеестру(Команда)
	...
	Если НайденоДокументовВ1С>0 Тогда 
		...
		Если Параметры.МодальностьЗапрещена Тогда
			Выполнить("ПоказатьВопрос(Новый ОписаниеОповещения(""ОбработчикСогласияПереотправки"", ЭтаФорма, ДопПараметрДляПередачиВОбработчик), ТекстВопроса, КнопкиВопроса,,,""EDI.Контур"")");
		Иначе
			РезультатВопроса = Вопрос(ТекстВопроса, КнопкиВопроса,,,"EDI.Контур");
...
Процедура Отчетность(Команда)
	...
	Если ЕстьМетиаданныеОтчетов() Тогда
	...
	Иначе
		Если Параметры.МодальностьЗапрещена = истина Тогда
			Выполнить("ПоказатьПредупреждение(,""В конфигурации не найдены необходимые объекты: регистр статистики и/или отчет.""+Символы.ПС+""Обновите конфигурацию с использованием отправленного вам файла обновления и инструкции."");");
		Иначе
			Предупреждение("В конфигурации не найдены необходимые объекты: регистр статистики и/или отчет."+Символы.ПС+"Обновите конфигурацию с использованием отправленного вам файла обновления и инструкции.",,"Контур.EDI Дополнительные возможности...");
			Отказ =истина;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Функция ЕстьМетиаданныеОтчетов()
