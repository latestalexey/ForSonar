...
Функция ПроверитьЗаполнениеПолей()
	...
	UniversalTransferDocumentId = СокрЛП(МодульОбъекта().ПолучитьЗначениеСвойстваОбъектаEDI(Сообщение.ДокументСсылка, "UniversalTransferDocumentId"));
	...
	Если Сообщение.Направление = "Входящее" Тогда
		...
		Если РежимРаботы = "Поставщик"
			...
			ОшибкиВалидации = МодульОбъекта().ПолучитьОшибкиВалидацииИзОбъектаСообщения(Сообщение.СообщениеСсылка);
...
Процедура ВидимостьДополнительныхДействийФормы(КнопкиДобавлены)
	...
	Если НЕ КнопкиДобавлены Тогда
		...
		Если Сообщение.Направление = "Исходящее"
			...
			ИЛИ МодульОбъекта().ПолучитьСтатусСообщения(Сообщение.ПереотправляемоеСообщениеСсылка) = "Доставлен с уточнением")
			Тогда
