...
Процедура СоздатьДеревоНастроек(ДеревоНастроек,ВидСправочника,СтруктураСвойств) Экспорт
	...
	Макет=МодульОбъекта().ПолучитьМакет("НастройкиПараметры");
	...
	Для Сч=1 По Область.ВысотаТаблицы Цикл
		НастройкаИД					=	МодульОбъекта().ПолучитьТекстЯчейкиОбласти(Область,Сч,1);
		НастройкаНаименование		=	МодульОбъекта().ПолучитьТекстЯчейкиОбласти(Область,Сч,2);
		ЭтоГруппа					=	МодульОбъекта().ПолучитьТекстЯчейкиОбласти(Область,Сч,3);
		ТекГруппа					=	МодульОбъекта().ПолучитьТекстЯчейкиОбласти(Область,Сч,4);
		_Тип						=	МодульОбъекта().ПолучитьТекстЯчейкиОбласти(Область,Сч,5);
//		_ТипВнутренний				=	МодульОбъекта().ПолучитьТекстЯчейкиОбласти(Область,Сч,6);//внутренний тип нужен только при заполнении таблицы свойств - в процедуре ДополнитьТаблицуСвойствИзМакета
		ЗначениеПоУмолчанию			=	МодульОбъекта().ПолучитьТекстЯчейкиОбласти(Область,Сч,7);
		Подсказка					=	МодульОбъекта().ПолучитьТекстЯчейкиОбласти(Область,Сч,8);
		ДляСвоихОбъектов			=	МодульОбъекта().ПолучитьТекстЯчейкиОбласти(Область,Сч,9);
		...
		Иначе
			...
			Если Лев(_Тип,7) = "_Список" Тогда
				...
				Нов.Список=ЗначениеВСтрокуВнутр(МодульОбъекта().ПолучитьСписокЗначенийИзМакета(Макет,_Тип));
...
Процедура ЗаписатьНастройкиСервер(СтруктураНастроек,Отказ)
	...
	СозранитьНастройкиСертификатовПоУмолчанию();
...
Процедура ИзменитьРасширение(Команда)
	НадписьРасширенияНедоступныПодсказкаНажатие("");
...
Процедура УдалитьРасширение(Команда)
	...
	Если Параметры.МодальностьЗапрещена Тогда
		Выполнить("ПоказатьВопрос(Новый ОписаниеОповещения(""ОбработчикСогласияУдаления"", ЭтаФорма, ДопПараметрДляПередачиВОбработчик), ТекстВопроса, КнопкиВопроса,,,""EDI.Контур"")");
	Иначе
...
Процедура ЗаполнитьНазначенныеСертификаты()
	...
	ЗначениеВРеквизитФормы(МодульОбъекта().СертификатыПоУмолчанию(),"СписокНазначенныхСертификатов");	
...
Процедура ЗаполнитьСертификатыПоУмолчаниюКлиент()
	...
	МассивСертификатов = ОсновнаяФорма().МодульОбменКлиент().МассивСертификатов();
...
&НаКлиенте
Процедура ИзменитьСертификатоПоУмолчанию(Команда)
	//Спросим серты стандартным выбором серта
	...
	Если Параметры.МодальностьЗапрещена Тогда 
		Выполнить("ОткрытьФормуОбъектаМодально(ПутьКФормам+""Сертификаты_СписокУправляемая"",ПараметрыФормы,""ОбработкаВыбораСертификатаПоУмолчанию"",,ЭтаФорма)");
	Иначе
		ВыбранноеЗначение=ПолучитьФормуОбработки("Сертификаты_СписокУправляемая",ПараметрыФормы,ЭтаФорма).ОткрытьМодально();
		ОбработкаВыбораСертификатаПоУмолчанию(ВыбранноеЗначение, неопределено);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаВыбораСертификатаПоУмолчанию(Результат=Неопределено,Доппараметр=Неопределено)
     Если Результат<>Неопределено Тогда 
	 	ЗаполнитьЗначенияСвойств(Элементы.СписокСертификатов.ТекущиеДанные,Результат);
	 КонецЕсли;
КонецПроцедуры // ОбработкаВыбораСертификатаПоУмолчанию()
...
&НаСервере
Процедура СозранитьНастройкиСертификатовПоУмолчанию()

	Для Каждого СтрокаСертификатаПоУмолчанию из СписокСертификатов Цикл
		МодульОбъекта().УстановитьЗначениеСвойстваОбъекта(СтрокаСертификатаПоУмолчанию.Организация, "ОтпечатокСертификата",СтрокаСертификатаПоУмолчанию.Отпечаток);
	КонецЦикла;
